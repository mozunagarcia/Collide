...
const recordSwipe = async (req, res) => {
  try {
    ...
    if (direction === "like") {
      ...
      if (mutual) {
        const matchCourse = sharedCourse || mutual.course || null;
        match = await Match.create({
          users: [swiperId, swipedUserId],
          ...(matchCourse && { course: matchCourse }),
        });
        await User.findByIdAndUpdate(swiperId, { $addToSet: { connections: swipedUserId } });
        await User.findByIdAndUpdate(swipedUserId, { $addToSet: { connections: swiperId } });
